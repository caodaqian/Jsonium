	...
	<div class="json-processor" :class="{ 'has-active-tab': !!activeTab }">
		...
		<div class="processor-header">
			...
			<TabBar
				...
				@select-tab="(id) => store.setActiveTab(id)"
				@close-tab="(id) => store.closeTab(id)"
				@rename-tab="(id, name) => store.updateTabName(id, name)"
				@new-tab="() => createNewTab()"
				@toggle-favorite="(id) => store.toggleFavorite(id)"
				@close-other-tabs="(id) => store.closeOtherTabs(id)"
				@close-all-tabs="() => store.closeAllTabs()"
				@close-left-tabs="(id) => store.closeLeftTabs(id)"
			/>
			<!-- 设置入口由状态栏提供，移除此处的重复按钮 -->
		</div>

		<div v-if="activeTab" class="processor-container">
			<div
			...
			>
				<template v-if="isNarrow">
					<div class="drawer-overlay" @click="store.editorSettings.controlPanelVisible = false" />
					<div class="control-panel-drawer">
						<ControlPanel
							:active-panel="activePanel"
							@panel-change="(p) => activePanel = p"
			...
			<div class="workspace">
				<div class="editor-section">
					<Editor
						...
						:auto-format="store.getEditorSettings().autoFormat"
						:debounce-ms="store.getEditorSettings().debounceMs || 300"
						:auto-format-on-idle="store.getEditorSettings().autoFormatOnIdle"
						:auto-format-on-paste="store.getEditorSettings().autoFormatOnPaste"
						@change="handleEditorChange"
					/>
				</div>
			</div>
			<DiffSidebar @open-line-diff="handleOpenLineDiff" @open-centered-diff="handleOpenCenteredDiff" />
		</div>

		<div v-if="showLineDiff" class="line-diff-overlay">
			<button class="line-diff-close" @click="closeLineDiff">
				关闭
			</button>
			<DiffView :left-content="lineLeft" :right-content="lineRight" />
		</div>

		<div v-if="showCenteredDiff" class="centered-diff-overlay" @click.self="closeCenteredDiff">
			<div
				class="centered-diff-panel"
				role="dialog"
				aria-modal="true"
				aria-label="居中对比结果"
			>
				<div class="centered-diff-header">
					...
					<button class="centered-diff-close" @click="closeCenteredDiff">
						关闭
					</button>
				</div>
				<div class="centered-diff-body">
					<DiffView :left-content="centeredLeft" :right-content="centeredRight" />
				</div>
			</div>
		</div>
		...
		<teleport to="body">
			<TableView
				...
				:json-content="activeTab.content"
				:array-path="store.tableView.arrayPath"
		...
		<StatusBar
			...
			@compare="handleCompare"
			@open-table-view="handleOpenTableView"
		/>

		<div v-else class="processor-empty">
